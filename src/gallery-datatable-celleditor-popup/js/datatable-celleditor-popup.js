...
//======================   PUBLIC METHODS   ===========================
        ...
        saveEditor: function(val){
            var cell = this.get('cell');
             //   rec  = cell.record, //this.get('record'),
             //   coln = cell.colKey; // this.get('colKey');
...
//======================   PRIVATE METHODS   ===========================
        ...
        //  Create the Overlay
        //
            var html = Y.Lang.sub(this.get('template'),{
            ...
            var overlay = new Y.Overlay(ocfg);
        ...
        //
            if(this.get('overlayContent')) {
                var cont = Y.Lang.sub(this.get('overlayContent'),{
                    classInput: this._classInput
                });
        ...
        _createInputCollection: function(overlay){
            ...
            var icont = Y.Lang.sub( icoll.wrapperContent,{
                classInput: this._classInput,
                items: itemhtml
            }) || '';
        ...
        _createOverlayButtons: function(overlay){
            ...
            Y.Array.each(ov_btns, function(btn){
                ...
                var btn_html = Y.Lang.sub(this.btnTemplate,{
                ...
                var btn_node = ov_ftr.appendChild( Y.Node.create(btn_html) );
        ...
        _onKeyEvent: function(e){
            ...
            if( dirxy ) {
                ...
                if(this._imRegex) {
                    ...
                    var RE = this._imRegex;

                    var newVal = cont.get('value') + char;
                    //  var RE = new RegExp(this._imRegex);
                    if(char && RE && !RE.test(newVal) )
                        e.preventDefault();
                }
        ...
        on: {
            ...
            editorShow : function(o){
                ...
                chks.each(function(n){
                    if(n && n.get('value')==val){
                        chk = n;
                        return true;
                    }
                    n.set('checked',false);
                });
                if(chk) chk.set('checked',true);
            }
        }
